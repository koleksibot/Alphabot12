...
gc_wa_lu = 'https://chat.whatsapp.com/GpS1GHhuMz2E7Pc5bBEOFX' //klo gk punya gc wa gk usah di ganti 👍
...
const bugtrol = { 
       ...
       const ftroli ={key: {fromMe: false,"participant":"0@s.whatsapp.net",   "remoteJid": "6289523258649-1604595598@g.us"  }, "message": {orderMessage: {itemCount: 2021,status: 200, thumbnail: fs.readFileSync(`image/${thumbnail}`), surface: 200, message: `Whatsapp Bot ☠\nBy ${ownername}`, orderTitle: 'Ꭰяѧċυʟʟѧ×͜×', sellerJid: '0@s.whatsapp.net'}},contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
        ...
        const fgif = {key: {participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6289643739077-1613049930@g.us" } : {})},message: {"videoMessage": { "title":`Whatsapp Bot ☠\nBy ${ownername}`, "h": `Hmm`,'seconds': '99999', 'gifPlayback': 'true', 'caption': `Whatsapp Bot ☠\nBy ${ownername}`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}
...
case 'setexif':
		    ...
		    case 'colong':
		            ...
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `Ꭰяѧċυʟʟѧ×͜×`
...
break
		...
		if (!mek.key.fromMe && !isGroupAdmins && !isOwner && !isCreator) return reply(lang.onlyOwner())
		     if (!isGroup) return fakegroup(lang.onlygc())
				...
				let infobopot = await getBuffer('https://telegra.ph/file/06fad83011a4b1cecd4ba.jpg')
							sendButLocation(from, infobot(status, offline,latensii, totalchat, giid, wa_version, mcc, mnc, os_version, device_manufacturer, device_model, ownerNumberrr,runtime, kyun) , `${enter}${enter}${botname} ☠ | By ${ownername}`,infobopot,  [{"buttonId": 'gcbot',"buttonText": {"displayText": "GROUP BOT"},"type": "RESPONSE"}], {contextInfo:{"mentionedJid": [ownerNumberrr]}})
							break
			case 'play2':
		...
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					fakethumb(buffer,'```Nih kak, jgn lupa PAP TETENYA YAH^_^```')
					fs.unlinkSync(ran)
					})
					break
        ...
        break;
           ...
           case 'getpp':
				if (mek.message.extendedTextMessage != undefined){
					...
					anuu  = {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${numbernye}@s.whatsapp.net`, 'remoteJid': '6289523258649-1604595598@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 Bot Ꭰяѧċυʟʟѧ×͜× 」`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}}
...
sendButImage(from,  `Nih udah jadi kak, jgn lupa donasi untuk beli dyno 🐦 ` , `${botname}™© | By ${ownername}`,photooxy, [{"buttonId": `owner`,"buttonText": {"displayText": "😈 Owner"},"type": "RESPONSE"},{"buttonId": `donasi`,"buttonText": {"displayText": "☠️ Donate"},"type": "RESPONSE"}], {thumbnail: Buffer.alloc(0), quoted: mek})
...
sendButImage(from,  `Nih udah jadi kak, jgn lupa donasi untuk beli dyno 🐦 ` , `${botname}™© | By ${ownername}`,photooxy2, [{"buttonId": `owner`,"buttonText": {"displayText": "😈 Owner"},"type": "RESPONSE"},{"buttonId": `donasi`,"buttonText": {"displayText": "☠️ Donate"},"type": "RESPONSE"}], {thumbnail: Buffer.alloc(0), quoted: mek})
await  limitAdd(sender, limit)
break         
...
sendButImage(from,  `Nih udah jadi kak, jgn lupa donasi untuk beli dyno 🐦 ` , `${botname}™© | By ${ownername}`,textpro, [{"buttonId": `owner`,"buttonText": {"displayText": "😈 Owner"},"type": "RESPONSE"},{"buttonId": `donasi`,"buttonText": {"displayText": "☠️ Donate"},"type": "RESPONSE"}], {thumbnail: Buffer.alloc(0), quoted: mek})
await  limitAdd(sender, limit)
break 
//1 TEXT  
case 'blackpink':
...
break
		  ...
		  case 'tebakin': case 'tebakgambar':
                    ...
                    alpha.sendMessage(from, tebakya, image, { quoted: mek, caption: "Jawaban salah Harus Pap Tetek/Kontol" })
                   await sleep(30000)
...
${petik}☠ Base : SELF-BOT${petik}
${petik}☠ Lib : Baileys, 3.5.2${petik}
${petik}☠ Prefix : ${prefix}${petik}
${petik}☠ Versi Bot : v0.0.7${petik}
...
${petik}☠ MCC : ${mcc}${petik}
${petik}☠ MNC : ${mnc}${petik}
${petik}☠ Baterai : ${isBattre}${petik}
${petik}☠ Charger : ${isCharge}${petik}
${petik}☠ Whatsapp : ${wa_version}${petik}
${petik}☠ Blockir : ${banned.length} Blocked${petik}
${petik}☠ Group Chat : ${totalgroup.length} Chat${petik}
${petik}☠ Personal Chat : ${totalkontak.length} Chat${petik}
${petik}☠ Total Chat : ${totalchat.length} Chat${petik}
${petik}☠ Speed : ${latensinya.toFixed(4)} Second${petik}
${petik}☠ Runtime : ${kyun(anuui)}${petik}`
fakeitem(teks)
}
...
2. Buka Sempakmu
3. Kocok Kontolmu.
4. Dan Buka Bokep Dihpmu.
5. Kocok Deh Sampe Crot`
...
if (budy.includes("eror",'error','Eror','Error')){
					...
					alpha.sendMessage(from, daieeeee, sticker, { contextInfo: {mentionedJid: [sender]}, quoted: { "key": { "participant": `${numbernye}@s.whatsapp.net`, "remoteJid":  '6283136505591-1614953337@g.us', "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": fs.readFileSync(`image/${thumbnail}`), "mimetype": "application/octet-stream","title": "Ꭰяѧċυʟʟѧ×͜×", "fileLength": "36", "pageCount": 0, "fileName": `Kenapa bisa error???`}}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		        }
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`Ꭰяѧċυʟʟѧ×͜× :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
